(function() {
  ...
  VPAIDCreative.prototype._dispatch = function(event) {
    if (this.events[event]) {
      try { this.events[event](event); } catch (e) { console.error("Dispatch error", event, e); }
    }
  };
  ...
  VPAIDCreative.prototype.initAd = function(width, height, viewMode, desiredBitrate, creativeData, environmentVars) {
    ...
    // Жёстко выставляем фулл-HD контейнер
    this.slot.style.width = "1920px";
    this.slot.style.height = "1080px";
    this.slot.style.position = "relative";
    this.slot.style.backgroundColor = "black";

    this.videoSlot.style.width = "1920px";
    this.videoSlot.style.height = "1080px";
    this.videoSlot.style.objectFit = "cover";
